...
//Wipes everything clean in the cohort and reset graph
    public void clear() {
    ...
    protected ArrayList<Cohort> sortMethod(ArrayList<Cohort> list, int block_size, boolean ascending, String attr) {
        ...
        {
            Cohort key = list.get(index);
            int move = index - 1;

            while(move >= 0) {
                Cohort c = list.get(move);
                int compare;

                if (attr.equalsIgnoreCase("name")) { compare = c.getModule().getName().compareToIgnoreCase(key.getModule().getName());
                }
                else
                {
                    compare = Integer.compare(c.getModule().getCode(), key.getModule().getCode());
                }
                boolean incorrect = ascending ? (compare > 0) : (compare < 0);
